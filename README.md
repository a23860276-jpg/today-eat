...
- **第一次打開 App 時，會自動帶入嶺東／南屯一帶的人氣店家**當作起始資料，可自由編輯或刪除。想重新帶入可到「設定 →重新帶入在地起始店家」。
  - 起始資料的車程是**概估值**，請依你的實際情況調整（尤其「車程距離」是相對你家的車程）。

### 2. 設定今天的條件（「決定」分頁）
- 大部分標籤是**點一下＝要**（綠）、**再點一下＝不要**（紅，排除）、**再點＝不限**。沒選的就不限制。例如把「麵」設成不要、把「晚餐」設成要。
- **車程距離**比較特別：它是「**最遠可接受**」的範圍——選一個就好，會**自動包含更近的**。例如不想開太遠就選「**10分鐘內**」，步行可到、5分鐘內的店也都會被算進來。
- 開啟時會依目前時間自動幫你選好餐別。
...
- 各欄位的可選值請對齊上面「建立店家」列出的標籤（例如車程距離要寫「10分鐘內」這種）。沒填的欄位留空即可，之後在 App 內補。
